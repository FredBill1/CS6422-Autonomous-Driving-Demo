...
from .dynamic_programming_heuristic import Node as DPNode
...
def calc_motion_inputs() -> Generator[tuple[float, int], None, None]:
...
def calc_rs_path_cost(reed_shepp_path: rs.Path) -> float:
    ...
    # steer penalty
    for course_type in reed_shepp_path.course_segment_types:
        if course_type != rs.CourseSegmentType.STRAIGHT:  # curve
            cost += STEER_COST * abs(MAX_STEER)

    # ==steer change penalty
    # calc steer profile
    n_course_segment_types = len(reed_shepp_path.course_segment_types)
    u_list = [0.0] * n_course_segment_types
    for i in range(n_course_segment_types):
        if reed_shepp_path.course_segment_types[i] == rs.CourseSegmentType.RIGHT:
            u_list[i] = -MAX_STEER
        elif reed_shepp_path.course_segment_types[i] == rs.CourseSegmentType.LEFT:
            u_list[i] = MAX_STEER

    for i in range(len(reed_shepp_path.course_segment_types) - 1):
...
def calc_cost(node: Node, h_dp: dict[int, DPNode], config: Config) -> float:
    ind = (node.y_index - config.min_y) * config.x_w + (node.x_index - config.min_x)
    if ind not in h_dp:
        return node.cost + 999999999  # collision cost
    return node.cost + H_COST * h_dp[ind].cost
...
def verify_index(node: Node, config: Config) -> bool:
    x_ind, y_ind = node.x_index, node.y_index
    if config.min_x <= x_ind <= config.max_x and config.min_y <= y_ind <= config.max_y:
        return True
...
def main():
    ...
    out_file = pathlib.Path("data/path.pkl")
    out_file.parent.mkdir(parents=True, exist_ok=True)
    with out_file.open("wb") as f:
        pickle.dump(dict(x=x, y=y, yaw=yaw), f)

    if show_animation:
        for i, (i_x, i_y, i_yaw) in enumerate(zip(x, y, yaw)):
            if i % 10 != 0:
                continue
            plt.cla()
            plt.plot(ox, oy, ".k")
